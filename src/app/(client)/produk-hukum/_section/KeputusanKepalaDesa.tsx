                    ...
                    <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        {legal.filter((item: LegalProduct) => item.type! == "village_head_decision").length > 0 ? legal.filter((item: LegalProduct) => item.type! == "village_head_decision").map((item: LegalProduct, i: number) => (
                            <tr key={i} className="bg-white  hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4">
                                    <a href={item.file} target="_blank" className="font-medium rounded-md px-2 py-1 text-sm bg-primary text-white hover:bg-primary/80" download={`${item.title}.${item.file.split('.').pop()}`}>Download</a>
                                </td>
                            </tr>
                        )) : (<tr>
