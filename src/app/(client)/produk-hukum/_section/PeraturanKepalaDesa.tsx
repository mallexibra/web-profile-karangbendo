  ...
  return (
    <div data-aos="fade-down" id="peraturankepaladesa">
      <ContainerClient classNames="pt-24">
        ...
        <p>
          Peraturan Kepala Desa adalah peraturan yang ditetapkan langsung oleh
          Kepala Desa untuk mengatur pelaksanaan teknis dari Peraturan Desa atau
          untuk mengatur hal-hal yang menjadi kewenangan Kepala Desa. Peraturan
          ini membantu menjalankan pemerintahan desa secara efektif dan efisien.
        </p>
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg mt-5 border border-primary">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {legal.filter(
                (item: LegalProduct) => item.type! == 'village_head_regulation',
              ).length > 0 ? (
                legal
                  .filter(
                    (item: LegalProduct) =>
                      item.type! == 'village_head_regulation',
                  )
                  .map((item: LegalProduct, i: number) => (
                    <tr key={i} className="bg-white  hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <a
                          href={item.file}
                          target="_blank"
                          className="font-medium rounded-md px-2 py-1 text-sm bg-primary text-white hover:bg-primary/80"
                          download={`${item.title}.${item.file.split('.').pop()}`}
                        >
                          Download
                        </a>
                      </td>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-4 text-center font-medium">
                    Data peraturan kepala desa sedang kosong!
                  </td>
                </tr>
              )}
            </tbody>
