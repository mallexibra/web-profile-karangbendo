...
export default function AddUMKM() {
  ...
  const getUser = async () => {
    ...
    const option = data
      .filter((user: User) => user.role == 'umkm')
      .map((user: User) => {
  ...
  type FormProduct = Pick<Product, 'name' | 'price' | 'description' | 'image'>;
            ...
            {item.product.length <= 0 ? (
              ...
              <div className="flex gap-3 items-start flex-wrap mt-3">
                {item.product.map((product: Product) => (
                  <div
                    key={i}
                    className="border border-custom bg-white flex flex-col justify-between rounded-md max-w-80 p-3"
                  >
                    <Image
                      src={product.image}
                      width={512}
