...
export default function PeraturanDesa() {
  ...
  const regulationSchema = yup.object({
    ...
    file: yup
      .mixed<File>()
      .nullable()
      .test('fileRequired', 'File wajib diisi', function (value: any) {
  ...
  const fetchData = async () => {
    try {
      ...
      const data = dataTemporary.filter(
        (legal: LegalProduct) => legal.type == 'village_regulation',
      );
  ...
  return (
    <Card>
      <div className="flex justify-between items-center mb-3">
        <p className="font-bold">Peraturan Desa</p>
        <button
          onClick={() => {
            setIsModalOpen(true);
          }}
      ...
      <dialog id={`modal_${type}_peraturandesa3`} className="modal">
        <div className="modal-box">
          ...
          >
            <LabelForm label="Judul Peraturan">
              <InputForm
                disabled={type == 'view'}
                {...register('title')}
                type="text"
