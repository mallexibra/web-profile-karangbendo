...
export default function KeputusanKepalaDesa() {
  ...
  const regulationSchema = yup.object({
    ...
    file: yup
      .mixed<File>()
      .nullable()
      .test('fileRequired', 'File wajib diisi', function (value) {
      ...
      .test(
        'fileFormat',
        'Format file tidak valid, hanya pdf, docx, dan doc yang diperbolehkan',
        function (value) {
          if (selectedFile != null) return true;
          if (value) {
  ...
  const fetchData = async () => {
    try {
      ...
      const data = dataTemporary.filter(
        (legal: LegalProduct) => legal.type == 'village_head_decision',
      );
    ...
    try {
      ...
      if (response.status) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Sukses!',
          text: 'Sukses delete data keputusan kepala desa',
        });
        fetchData();
      } else {
        close();
  ...
  const handleAddRegulation = async (data: any) => {
    ...
    try {
      ...
      formData.append('type', 'village_head_decision');
      ...
      if (response.status) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Sukses!',
          text: `Sukses ${id ? 'edit' : 'tambah'} data keputusan kepala desa`,
        });
        fetchData();
      } else {
        close();
  ...
  return (
    <Card>
      <div className="flex justify-between items-center mb-3">
        <p className="font-bold">Keputusan Kepala Desa</p>
        <button
          onClick={() => {
            setIsModalOpen(true);
          }}
            ...
            <div className="relative">
              {selectedFile && (
                <IconSquareRoundedXFilled
                  ...
                  className="text-red-600 absolute top-4 -right-2 z-10 cursor-pointer"
                />
              )}
                ...
                <tr key={i}>
                  ...
                  <td>
                    <div className="flex items-center gap-3">
                      <a
                        href={item.file}
                        download={`${decodeURIComponent((item.file.split('/').pop()).split('_')[1])}`}
                        target="_blank"
                      >
                      ...
                      <IconEdit
                        onClick={() => {
                          ...
                          setIsModalOpen(true);
                          setSelectedFile(
                            `${decodeURIComponent((item.file.split('/').pop()).split('_')[1])}`
                          );
                        }}
