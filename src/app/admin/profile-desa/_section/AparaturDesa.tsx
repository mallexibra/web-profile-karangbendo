...
export default function AparaturDesa() {
  ...
  const villageApparatusSchema = yup.object({
    ...
    profile: yup
      .mixed<File>()
      .test('fileRequired', 'Foto Profile wajib diisi', function (value) {
    ...
    try {
      ...
      if (response.status) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Sukses!',
          text: `Sukses ${id ? 'edit' : 'tambah'} data apparatur desa`,
        });
        fetchData();
      } else {
        close();
            ...
            {selectedImage || dataImage ? (
              <div className="relative">
                <Image
                  ...
                  className="rounded-md w-full h-[320px] object-cover object-top"
                />
                <IconSquareRoundedXFilled
                  onClick={() => {
                    setSelectedImage(null);
