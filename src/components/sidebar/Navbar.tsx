...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const headerHeight = !path.includes(pathname)
        ? window.innerHeight
        : headerRef.current?.offsetHeight || 200;
  ...
  return (
    <nav
      className={cn(
        'py-3 text-white hover:bg-primary border-b border-white/20 fixed z-50 top-0 left-0 right-0 transition-colors duration-300',
        {
    ...
    >
      <ContainerClient classNames="flex flex-col md:flex-row justify-between items-center gap-3">
        <div className="flex w-full md:hidden justify-between items-center">
          <Link href={'/'} className="flex justify-start items-center gap-2">
            ...
            <h1 className="text-lg font-extrabold">
              Desa <br className={cn(pathname == '/auth/login' && 'hidden')} />{' '}
              Karangbendo
            </h1>
          </Link>
          <button
        ...
        <div
          className={cn('flex md:hidden flex-col gap-2 mt-3 mb-3', {
            hidden: isOpen,
          })}
        >
          <div className="flex flex-col justify-center items-center gap-3">
            <div
              className={cn(
                'relative group flex justify-center items-center cursor-pointer gap-2',
                { hidden: isOpen && windowWidth < 768 },
              )}
            >
            ...
            <div className="relative group flex justify-center items-center cursor-pointer gap-2">
              <Link
                href={'/informasi-desa'}
                className={cn(
                  pathname == '/informasi-desa' && 'font-semibold',
                  { hidden: isOpen && windowWidth < 768 },
                )}
              >
                Informasi Desa
              ...
              <ul className="absolute top-4 w-max left-0 hidden md:group-hover:block bg-primary backdrop-blur shadow-lg mt-2 p-2 rounded-md">
                ...
                <li className="py-1 px-2 hover:bg-gray-100">
                  <Link href={'/informasi-desa#infrastrukturdesa'}>
                    Infrastruktur Desa
                  </Link>
                </li>
                <li className="py-1 px-2 hover:bg-gray-100">
                  <Link href={'/informasi-desa#pengaduanmasyarakat'}>
            ...
            <div className="relative group flex justify-center items-center cursor-pointer gap-2">
              <Link
                href={'/transparansi'}
                className={cn(pathname == '/transparansi' && 'font-semibold', {
                  hidden: isOpen && windowWidth < 768,
                })}
              >
                Transparansi
              ...
              <ul className="absolute top-4 w-max left-0 hidden md:group-hover:block bg-primary backdrop-blur shadow-lg mt-2 p-2 rounded-md">
                ...
                <li className="py-1 px-2 hover:bg-gray-100">
                  <Link href={'/transparansi#dokumentasikegiatan'}>
                    Dokumentasi Kegiatan
                  </Link>
                </li>
                <li className="py-1 px-2 hover:bg-gray-100">
                  <Link href={'/transparansi#renacanakerjadananggaran'}>
            ...
            <div className="relative group flex justify-center items-center cursor-pointer gap-2">
              <Link
                href={'/produk-hukum'}
                className={cn(pathname == '/produk-hukum' && 'font-semibold', {
                  hidden: isOpen && windowWidth < 768,
                })}
              >
                Produk Hukum
              ...
              <ul className="absolute top-4 w-max left-0 hidden md:group-hover:block bg-primary backdrop-blur shadow-lg mt-2 p-2 rounded-md">
                <li className="py-1 px-2 hover:bg-gray-100">
                  <Link href={'/produk-hukum#peraturandesa'}>
                    Peraturan Desa
                  </Link>
                </li>
                <li className="py-1 px-2 hover:bg-gray-100">
                  <Link href={'/produk-hukum#peraturankepaladesa'}>
                    Peraturan Kepala Desa
                  </Link>
                </li>
                <li className="py-1 px-2 hover:bg-gray-100">
                  <Link href={'/produk-hukum#keputusankepaladesa'}>
            ...
            <Link
              href="/produk-umkm"
              className={cn(
                pathname.startsWith('/produk-umkm') && 'font-semibold',
                { hidden: isOpen && windowWidth < 768 },
              )}
            >
              Produk UMKM
          ...
          >
            <Link
              href={'/auth/login'}
              className={`flex justify-center items-center gap-3 px-4 py-2 md:text-base text-sm font-medium rounded-md ${
                isNavHovered
        ...
        <div className="md:flex text-sm items-center w-full overflow-hidden justify-between gap-3 hidden">
          ...
          <div className="flex flex-col md:flex-row justify-center items-center gap-3">
            ...
            <div className="relative group flex justify-center items-center cursor-pointer gap-2">
              <Link
                href={'/informasi-desa'}
                className={cn(pathname == '/informasi-desa' && 'font-semibold')}
              >
                Informasi Desa
              </Link>
              <IconChevronDown className="w-4 hidden md:block" />
          ...
          <div className={cn(pathname == '/auth/login' && 'hidden')}>
            <Link
              href={'/auth/login'}
              className={`flex justify-center items-center gap-2 px-4 py-2 text-sm font-medium rounded-md ${
                isNavHovered
                  ? 'bg-white text-primary hover:bg-white/80'
                  : isScrolled
                    ? 'bg-white text-primary hover:bg-white/80'
