  ...
  ({ label, name, type, placeholder, ...props }, ref) => {
    ...
    if (type === 'text' || type === 'number' || type === 'date') {
      return (
        <input
          type={type}
          className={`block w-full px-2 py-3 border-custom border text-xs rounded-md outline-none 
                        ${props.disabled ? 'bg-slate-100 cursor-not-allowed' : 'bg-second'}`}
    ...
    return (
      <div
        className={`w-full px-2 py-5 border-custom border text-xs font-semibold text-center rounded-md outline-none transition-colors duration-300 ${
          isDragging
            ? 'bg-blue-100 border-dashed border-primary text-primary'
            : 'bg-second'
        }`}
